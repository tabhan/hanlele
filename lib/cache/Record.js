...
import {emitRecordEvent, listenToRecordEvent} from './dispatcher';
...
export default class Record {
    ...
    constructor(opt) {
        ...
        _(this.requiredCaches).each(key => {
            logger.info('register change event', key);
			listenToRecordEvent(key, 'change', e => {
                logger.debug('will reload', this.key);
                this.reload();
            });
        });
    }
    ...
    globalGet() {
        return this.cache ? this.cache.get.apply(this.cache, arguments) : undefined;
    }
